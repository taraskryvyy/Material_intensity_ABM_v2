...
# try:
#     df = df.drop(index=pd.IndexSlice["Sharply declining ore cost", 63])
# except KeyError:
#     pass
...
# 'Metal GDP'
# metal_gdp_df = df.loc[['Metal GDP']]
# plt.figure(figsize=fig_size)
# sns.lineplot(x='Timestep Number',
#                 y='Value',
#                 data=metal_gdp_df,#df.loc[["Metal GDP"]],
#                 hue='Scenario',
#              errorbar=errorbar_format
#              )
# plt.title('Metal GDP')
# plt.xlabel('Timestep Number')
# plt.ylabel('Metal GDP')
# if save_figs:
#     plt.savefig('scenario_metal_gdp.'+format)
# if show_figs:
#     plt.show()
# plt.close()
...
# 'Metal capital GDP'
# metal_capital_gdp_df = df.loc[['Metal capital GDP']]
# plt.figure(figsize=fig_size)
# sns.lineplot(x='Timestep Number',
#                 y='Value',
#                 data=metal_capital_gdp_df,#df.loc[["Metal capital GDP"]],
#                 hue='Scenario',
#              errorbar=errorbar_format
#              )
# plt.title('Metal Capital GDP')
# plt.xlabel('Timestep Number')
# plt.ylabel('Metal Capital GDP')
# if save_figs:
#     plt.savefig('scenario_metal_capital_gdp.'+format)
# if show_figs:
#     plt.show()
# plt.close()

'Average metal buffer'
average_metal_buffer_df = df.loc[["Average metal buffer"]]
if "Average metal buffer" in smooth_figs:
    smooth(average_metal_buffer_df, smooth_window)
plt.figure(figsize=fig_size)
sns.lineplot(x='Timestep Number',
                y='Value',
                data=average_metal_buffer_df,#df.loc[["Average metal buffer"]],
                hue='Scenario',
             errorbar=errorbar_format
             )
plt.title('Average Metal Buffer')
plt.xlabel('Timestep Number')
plt.ylabel('Average Metal Buffer')
if save_figs:
    plt.savefig('scenario_average_metal_buffer.'+format)
if show_figs:
    plt.show()
plt.close()
